...
export const api = {
  ...
  productContext: (id) => request(`/me/products/${id}/context`),
  saveProductContext: (id, data) => request(`/me/products/${id}/context`, json('PUT', data)),
  ...
  runAIPipeline: (productId) => request(`/ai/pipeline/${productId}`, { method: 'POST' }),
  stories: (savedOnly = false) => request(`/stories?saved_only=${savedOnly}`),
  updateStory: (id, data) => request(`/stories/${id}`, json('PATCH', data)),
  generateStoryProposals: (productId) => request(`/story-proposals/generate/${productId}`, { method: 'POST' }),
  selectStoryProposal: (id) => request(`/story-proposals/${id}/select`, { method: 'PATCH' }),
  selectStoryWithAI: (id) => request(`/ai/story-proposals/${id}/select`, { method: 'PATCH' }),
  proposalExperiences: (id) => request(`/story-proposals/${id}/experiences`),
