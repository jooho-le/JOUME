...
export default function AppShell({pageId,navigate,children}){
  ...
  return <div className="ia-site">
    <header className="ia-header"><button className="ia-logo" onClick={()=>navigate('product-discovery')}>MCM</button><nav>{GLOBAL_NAV.map(([label,id])=><button className={pageId===id?'on':''} onClick={()=>navigate(id)} key={id}>{label}</button>)}</nav><button className="ia-product-trigger" onClick={()=>setSelector(!selector)}><img src={visetosBackpack}/><span>STARK BACKPACK<small>현재 제품 변경</small></span><b>⌄</b></button></header>
